...
/// 最も左の葉ノードを探索する
pub fn leftmost_leaf<const D: usize, K, V>(x: Option<NodePtr<D, K, V>>) -> Option<NodePtr<D, K, V>>
where
    [(); 2 * D - 1]:,
{
    let mut x = x?;
...
/// 最も右の葉ノードを探索する
pub fn rightmost_leaf<const D: usize, K, V>(x: Option<NodePtr<D, K, V>>) -> Option<NodePtr<D, K, V>>
where
    [(); 2 * D - 1]:,
{
    let mut x = x?;
...
mod test {
    ...
    fn test_leftmost_leaf() {
        let tree: Option<Rc<RefCell<BTreeNode<2, i32, &str>>>> = btree! {
        ...
        let leftmost = leftmost_leaf(tree.clone());
        let rightmost = rightmost_leaf(tree.clone());
